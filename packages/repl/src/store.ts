...
import * as defaultCompiler from 'vue/compiler-sfc' //把单文件组件编译成为js代码
...
export function useStore(
  {
    files = ref(Object.create(null)), // 文件数组
    activeFilename = undefined!, // 当前文件名
    mainFile = ref('src/App.vue'), // 主文件路径
    template = ref({
      welcomeSFC: welcomeSFCCode, //主文件的组件
      newSFC: newSFCCode, //新建文件的组件
    }),
    builtinImportMap = undefined!, // vue的一些cdn

    errors = ref([]), // 错误数组
    showOutput = ref(false), // 显示输出
    outputMode = ref('preview'), // 输出模式
    sfcOptions = ref({}), // 组件选项
    compiler = shallowRef(defaultCompiler), //单文件组件编译器
    vueVersion = ref(null), //vue 版本

    locale = ref(),
    typescriptVersion = ref('latest'), // ts 版本
    dependencyVersion = ref(Object.create(null)), // 依赖版本
    reloadLanguageTools = ref(), // 重新加载语言工具
  }: Partial<StoreState> = {},
